...
def ScrambleChunks(x, fs):
    ...
    while arrayIndices:
        ...
        startTime = onsetTimes[currChunk] * fs
        if currChunk == (len(onsetTimes) - 1):
            endTime = len(x) - 1
        else:
            endTime = onsetTimes[currChunk+1] * fs
        
        currAudio = _Window(x[startTime:endTime])
        
        if (len(currAudio) < .13 * fs) and (len(currAudio) > .01 * fs) and (random.random() < .15):
...
def Run(theFile):
    ...
    for track, choices, seed, i in zip(tracks, eventChoices, seedRhythms, range(numTracks)):
        ...
        for choice in choices:
            if choice == 0:
                ...
                # Scramble it up! via chunks
                print "Syncopate 17"
                transAudio = Syncopate(seed, fs, 17)
    ...
    essentia.standard.MonoWriter(filename="outputs/%s.wav" % trackName, format="wav")(scramblaudio)
    return scramblaudio


if __name__ == "__main__":
	fileName = 'wavs/bbf3.wav'
